...
function isInsideCircle(circle, point) {
  const distance = (p1, p2) => (p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2;
  if (distance(circle.center, point) >= circle.radius ** 2) return false;
  return true;
}
...
function isCreditCardNumber(ccn) {
  const arr = String(ccn).split('').map((el) => +el);
  const m = Array(arr.length)
    .fill(1)
    .map((_, index) => ((index % 2) ? 2 : 1))
    .reverse();
  const s = arr
    .map((el, index) => el * m[index])
    .map((el) => ((el < 10) ? el : el - 9))
...
function getDigitalRoot(num) {
  let sum = num;
  do {
    const arr = String(sum).split('').map((el) => +el);
...
function isBracketsBalanced(str) {
  ...
  const close = [']', ')', '}', '>'];
  let change = 1;
  while (change > 0 && arr.length > 0) {
    change = 0;
    for (let i = 0; i < arr.length; i += 1) {
      const pos = open.indexOf(arr[i]);
      if ((pos !== -1) && (arr[i + 1] === close[pos])) {
        change = 1;
        arr.splice(i, 2);
        break;
      }
...
function evaluateTicTacToePosition(pos) {
  ...
  // 9
  //   [[ ???, ???, ???],
  //    [ ???, ???, ???],       =>  undefined, nothing of 1-8 situations did not work
  //    [ ???, ???, ???]]
  return undefined;
}
